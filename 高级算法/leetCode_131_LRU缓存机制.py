...
思路:
使用 哈希 + 双向链表
    哈希用来存储链表中的节点,这样可以做到O(1)的查找
    同时,为了做到O(1)的移动节点,我们使用链表来实现
    我们定义一个伪头部与伪尾部在链表中,这样在进行操作时,我们的链表的首尾部不会发生变化.
    我们将伪头部的下一个节点,定义为最近使用的节点,伪尾部的前一个节点,定义为最久未使用节点
    那么在进行插入时,如果缓存已满,要删除的就是尾节点的前一个节点,而在使用节点后,要将节点
    移动到首节点的下一个节点..
"""
